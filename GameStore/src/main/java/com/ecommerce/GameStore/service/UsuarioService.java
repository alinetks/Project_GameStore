...
import com.ecommerce.GameStore.model.UsuarioLogin;
...
public Optional<Usuario> atualizarUsuario(Usuario usuario) {
		...
		if(usuarioRepository.findById(usuario.getId()).isPresent()) {
			...
			if (calcularIdade(usuario.getNascimento()) < 18)
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Usuario menor de idade. ", null);
	...
	public Optional<UsuarioLogin> autenticarUsuario(Optional<UsuarioLogin> usuarioLogin){
		...
		if(buscaUsuario.isPresent()) {
			...
			if(compararSenhas(usuarioLogin.get().getSenha(), buscaUsuario.get().getSenha())) {
				...
				usuarioLogin.get().setNascimento(buscaUsuario.get().getNascimento());
